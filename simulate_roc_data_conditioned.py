...
def run_simulations(reps, pop_size, n, recombination_rate, seg_sites, growth_rate, timeg, freq,
            SAA, SaA, events_file, tree_law, mfilename, concentration, projdir, print_history):
    if SAA is not None:
        if SaA is None:
            SaA = SAA / 2
    ...
    if SAA is not None:
        SAA_scaled = SAA * 2 * pop_size
        SaA_scaled = SaA * 2 * pop_size
        t = timeg / (4 * pop_size)
        msms_cmd = msms_cmd + ["-SI", str(t), "1", str(freq),  "-SAA", str(SAA_scaled),
                               "-SaA", str(SaA_scaled)]
...
@click.option('-t', '--timeg', type=float, help='Time of onset of selection in generations.')
@click.option('-f', '--freq', type=float, help='Allele frequency at onset of selection.')
...
def main(reps, job_no, pop_size, n, length, mutation_rate, seg_sites, growth_rate, sho, she, timeg, freq,
    ...
    trs, taj_D, sfs_list = run_simulations(reps, pop_size, n, recombination_rate, seg_sites, 0,
        timeg, freq, None, None, None, tree_law, mfilename, concentration, projdir, print_history)
    ...
    trs, taj_D, sfs_list = run_simulations(reps, pop_size, n, recombination_rate, seg_sites, growth_rate,
        timeg, freq, sho, she, events_file, tree_law, mfilename, concentration, projdir, print_history)
