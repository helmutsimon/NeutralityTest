...
""" Calculates neutrality odds and Tajima's D for neutral and non-neutral data simulated
    with msprime or MSMS if selection is involved. This version simulates using a value of theta
     (i.e. mutation rate), as opposed to conditioning on the number of segregating sites.
...
def test_neutrality2(sfs, random_state=None, reps=10000):
    """Calculate the odds ratio for neutral / not neutral."""
    n = len(sfs) + 1
    j_n = np.diag(1 / np.arange(2, n + 1))
    erm = get_ERM_matrix(n)
